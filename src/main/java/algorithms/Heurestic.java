...
public class Heurestic {
	...
	public  int heurestic_function(int[][] state) {
		set.clear();
		int wins = 3*7 + 4*6 + 12*2 ;
		for(int i = 0 ; i < 6 ; i++ ) {
			for(int j = 0 ; j < 7;j++) {
				if(state[i][j] == 2) {
					check_horzential(i, j);
					check_vertical(i, j);
					check_diagonal(i, j);
				}
			}
	...
	private void check_horzential(int i , int j) {
		String s = "";
		if(j>=0 && j<=3) {
			s="h"+i+"0";
			set.add(s);
	...
	public static void main(String[] args) {
		...
		state[2][1]=2;
		state[2][3]=1;
		Heurestic h = new Heurestic();
		int a = h.heurestic_function(state);
